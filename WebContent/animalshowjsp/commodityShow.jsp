...
<head>
    ...
    <script type="text/javascript" src="${pageContext.request.contextPath}/js/xiala.js"></script>
...
<div class="focus">
    <div class="images">
        <div class="item"><img src="${pageContext.request.contextPath}/images/bg1.jpg" class="img"/></div>
        <div class="item"><img src="${pageContext.request.contextPath}/images/4.jpg" class="img"/></div>
        <div class="item"><img src="${pageContext.request.contextPath}/images/bg3.jpg" class="img"/></div>
    </div>
    <div class="title">
        <p>请用心对待每一个你领养的宠物</p>
        <p>你的人生很长，它的人生很短</p>
...
<div style="width: auto;height: 60%;font-size: 16px; margin-top: 20px;">
    <div style=" font-family: 华文彩云;font-size: 18px;background-color: antiquewhite;margin-left: 2%;line-height: 39px;">
        <span style="color: #ffc001">商品区</span></div>
    <table width="100%" style="margin-left: 5%">
        ...
        <tr style="width: 100%">
            <c:forEach items="${sessionScope.commodity.lists}" var="commodity">
                <td>
                    <div style="width: 25%; display:inline;line-height: 100%;"><a
                            href="loadAnimalJsp.do?p_kid=${commodity.p_kid}">${commodity.p_name}
                        <br/>描述：${commodity.p_bodytype} <br/>价格：${commodity.p_price} <br/><img
                                type="image" src="${commodity.p_path}" width="220" height="220" alt=""></a></div>
                </td>
            </c:forEach>
        </tr>
        <tr>
            <td colspan="4">
                <span>第${sessionScope.commodity.currPage+1}/ ${sessionScope.commodity.totalPage}页</span>
                <span>总记录数：${sessionScope.commodity.totalCount }  每页显示:${sessionScope.commodity.pageSize}</span>
                <span>
               <c:if test="${sessionScope.commodity.currPage != 0}">
                   <a href="${pageContext.request.contextPath }/loadCommodityInfo.do?currentPage=0&p_kind=${p_kind}">[首页]</a>
                   <a href="${pageContext.request.contextPath }/loadCommodityInfo.do?currentPage=${sessionScope.commodity.currPage-1}&p_kind=${p_kind}">[上一页]</a>
               </c:if>

               <c:if test="${sessionScope.commodity.currPage+1!= sessionScope.commodity.totalPage}">
                   <a href="${pageContext.request.contextPath }/loadCommodityInfo.do?currentPage=${sessionScope.commodity.currPage+1}&p_kind=${p_kind}">[下一页]</a>
                   <a href="${pageContext.request.contextPath }/loadCommodityInfo.do?currentPage=${sessionScope.commodity.totalPage-1}&p_kind=${p_kind}">[尾页]</a>
               </c:if>
            </td>
